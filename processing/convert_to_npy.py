...
Reads snapshots from one simulation run, merges ranks, assembles 2D fields,
optionally downsamples, and saves as .npy + metadata .npz.

Designed for the multi-seed workflow: run once per seed directory.

Examples:
    # Single seed, snaps 50-500 (orbits 5-50), native 256x256
    ...
    # All snapshots at lower resolution
    python processing/convert_to_npy.py \
        --bindir .../bin --outdir .../npy --resolution 128

    # Subset of fields
    python processing/convert_to_npy.py \
        --bindir .../bin --outdir .../npy --fields velx,vely,Bx

    # Batch all seeds (shell loop):
...
def discover_snapshots(bindir, vargroup="mhd_w"):
    ...
    if not os.path.isdir(rank0):
        # Single-rank run: bins might be directly in bindir
        rank0 = bindir
...
def read_snapshot_fields(bindir, snap_num, field_defs):
    ...
    for vargroup, field_list in groups.items():
        ...
        if not os.path.exists(rank0_path):
            # Try without rank subdirectory (single-rank runs)
            rank0_path = os.path.join(bindir, f"HB3.{vargroup}.{snap_str}.bin")
            if not os.path.exists(rank0_path):
                raise FileNotFoundError(
                    f"Missing: HB3.{vargroup}.{snap_str}.bin in {bindir}"
                )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to bin/ directory containing rank_* subdirs (or bins directly)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Target spatial resolution. Default: native resolution (no downsampling)",
    )
    parser.add_argument(
        "--snap-start", type=int, default=None,
        help="First snapshot index (default: first available)",
    )
    parser.add_argument(
        "--snap-end", type=int, default=None,
        help="Last snapshot index, inclusive (default: last available)",
    )
    parser.add_argument(
    ...
    if not all_snaps:
        print(f"ERROR: No snapshots found in {args.bindir}")
        sys.exit(1)
    ...
    n_snaps = len(snaps)
    if n_snaps == 0:
        print(f"ERROR: No snapshots in range [{snap_start}, {snap_end}] with stride {args.snap_stride}")
        sys.exit(1)
    print(f"Processing {n_snaps} snapshots: {snaps[0]:05d}..{snaps[-1]:05d} "
          f"(stride={args.snap_stride})")

    # Determine resolution from first snapshot
    snap_str = f"{snaps[0]:05d}"
    rank0_path = os.path.join(args.bindir, "rank_00000000", f"HB3.mhd_w.{snap_str}.bin")
    if not os.path.exists(rank0_path):
        rank0_path = os.path.join(args.bindir, f"HB3.mhd_w.{snap_str}.bin")
    fd0 = bin_convert.read_all_ranks_binary(rank0_path)
    native_res_x = fd0["Nx1"]
    native_res_y = fd0["Nx2"]

    res = args.resolution if args.resolution is not None else native_res_x
    print(f"Native resolution: {native_res_x}x{native_res_y}")
    print(f"Output resolution: {res}x{res}")
